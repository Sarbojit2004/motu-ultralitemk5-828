...
const REEL = process.argv.find((a) => a === "1" || a === "2") ?? process.env.REEL ?? "1";
const S = JSON.parse(fs.readFileSync(
  path.resolve(__dirname, "..", "src", `schedule-reel${REEL}.json`), "utf8"));
const M = JSON.parse(fs.readFileSync(path.resolve(__dirname, "..", "src", "manifest.json"), "utf8"));

// A reel is checked against ITS OWN portion of the division, not the full
// inventory — the pair together must account for all 69, which check-pair.mjs
// verifies independently.
const ALL = M.images.filter((i) => i.reel === Number(REEL)).map((i) => i.idx);
...
console.log(`REAL-IMAGE COVERAGE — ${S.id} (${S.title})`);

...
const S = JSON.parse(fs.readFileSync(path.join(ROOT, "src", `schedule-reel${REEL}.json`), "utf8"));
const OUT = process.env.QA_OUT ?? path.join(ROOT, "out", (process.argv.includes("--resolve") ? "qa-resolve" : "qa") + `-reel${REEL}`);
...
const only = args.filter((a) => !a.startsWith("--") && a !== "1" && a !== "2");

...
const CHIPS: Record<string, string[]> = {
  hook: [],
  shared: ["ESS SABRE32 ULTRA · ES9026PRO", "125 dB(A) DYNAMIC RANGE", "+74 dB GAIN · 1 dB STEPS", "−129 dBu EIN", "CueMix 5 · UP TO 192 kHz"],
  sul: ["2 × COMBO MIC / LINE / INST", "18 IN / 22 OUT · 40 CHANNELS", "HALF-RACK · EXTERNAL PSU", "8 CH OPTICAL · 1 BANK", "1 × PHONES · BALANCED TRS MAIN"],
  s828: ["2 × COMBO MIC / LINE / INST", "28 IN / 32 OUT · 60 CHANNELS", "1U RACK · INTERNAL PSU", "16 CH OPTICAL · 2 BANKS", "3.9 in RGB LCD · TALKBACK · A/B"],
  together: [],
  close: [],
};
...
export const Film: React.FC<FilmProps> = ({ canvas, segments, pins, outroAt, bed, vo }) => {
  ...
  // The three shared-platform numbers, each anchored to the caption that says
  // it, and the UltraLite-mk5's round-trip figure anchored to its own line.
  const T_DR = capStart("shared", "125 dB") - 0.15;
  const T_GAIN = capStart("shared", "74 dB") - 0.15;
  const T_EIN = capStart("shared", "129") - 0.15;
  const T_RTL = capStart("sul", "2.4 ms") - 0.15;
  ...
  const TopBlock: React.FC = () => {
    ...
    if (seg.id === "shared") {
      // Latest-first, so each number replaces the one before it as it is spoken.
      if (t >= T_EIN && T_EIN > 0) demo = <ScaleMeter title="EQUIVALENT INPUT NOISE" value={-129} min={-140} max={-100} unit="dBu" product="shared" f={frame - sec(T_EIN)} width={width} s={S} />;
      else if (t >= T_GAIN && T_GAIN > 0) demo = <ScaleMeter title="PREAMP GAIN" value={74} min={0} max={80} unit="dB" product="shared" f={frame - sec(T_GAIN)} width={width} s={S} />;
    ...
    } else if (seg.product === "pul" || seg.product === "p828") {
      demo =
        seg.id === "sul" && t >= T_RTL && T_RTL > 0 ? (
          <LatencyTrace product={seg.product} f={frame - sec(T_RTL)} width={width} s={S} value="2.4" />
        ) : (
          <IoLadder product={seg.product} f={f} width={width} s={S} />
        );
    } else if (seg.id === "together") {

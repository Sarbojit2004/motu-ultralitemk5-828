#!/usr/bin/env python3
"""Writes the separated audio deliverables, exactly the length of the reel:

  out/audio/motu-ultralite-828-<name>-music-bed.wav        the continuous bed, music only
  out/audio/motu-ultralite-828-<name>-transition-sfx.wav   every SFX cue at its exact frame,
                                                 music fully silent
...
sheet in out/audio/motu-ultralite-828-<name>-sfx-plan.json gives — the same sheet
...
for film in ("reel",):
    plan = json.load(open(os.path.join(OUT, f"motu-ultralite-828-{film}-sfx-plan.json")))
    ...
    if peak > 0.98:  # overlapping cues can sum past full scale; guard, never clip
        sfx *= 0.98 / peak
    p = os.path.join(OUT, f"motu-ultralite-828-{film}-transition-sfx.wav")
    write_wav(p, sfx)
    ...
    p = os.path.join(OUT, f"motu-ultralite-828-{film}-music-bed.wav")
    write_wav(p, bed)

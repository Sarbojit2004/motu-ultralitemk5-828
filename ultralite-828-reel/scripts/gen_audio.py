#!/usr/bin/env python3
"""Synthesises every audio asset for the MOTU UltraLite-mk5 & 828 reel.
...
widening — the same class of DSP the M-Series reel used, but with new material:
  ...
  low end           sine sub                     a soft kick on the beat

One continuous 90.000 s bed is produced,
...
# (energy, brightness, arp, pulse, kick) per segment of the reel's own script.
# The two acts are deliberately given different characters rather than the same
# curve twice: the UltraLite-mk5 runs brighter and lighter on its feet, the 828
# runs weightier and darker — the same contrast the teal/amber accents carry.
ZONE = {
    "hook":     (0.50, 0.78, False, True,  False),
    "shared":   (0.68, 1.00, True,  True,  False),
    "sul":      (0.78, 1.12, True,  True,  True),
    "s828":     (0.80, 0.98, True,  True,  True),
    "together": (0.88, 1.18, True,  True,  True),
...
if __name__ == "__main__":
    ...
    for name, key, dur, kick_on in (("music-bed-reel", "reel", 90.0, True),):
    ...
    print("\n  vo-reel.wav — a silent placeholder. Replace it with the recorded take.")
    print("done.")

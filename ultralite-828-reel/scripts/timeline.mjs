...
import { REEL_TL, REEL_WPM } from "../src/script.ts";

const show = (name, tl, runtime) => {
  console.log(`\n== ${name}: ${tl.words} spoken words, narration ends ${tl.total.toFixed(2)} s of ${runtime} s, effective ${(tl.words / (tl.total / 60)).toFixed(1)} wpm (written ${REEL_WPM})`);
...
writeFileSync("scripts/_timeline.json", JSON.stringify({ reel: strip(REEL_TL) }, null, 1));
